...
static std::string createID()
{
    ...
    unsigned volumeHash = getVolumeHash();
   //  unsigned macHash = getMacHash();
    std::string rawID = machineName+std::to_string(cpuHash)+std::to_string(volumeHash);
    checkPermission *controller = new checkPermission();
    auto id = controller->generateMD5(&rawID[0]);
    return id;
}
   }
}
